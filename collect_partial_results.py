...
def make_task_dict(task_name, subtask_name, preds_path):
    ...
    if not os.path.exists(preds_path):
        logging.warning(f"Error: no predictions found for \"{subtask_name}\" (in {task_name}).")
        return dict()
...
if __name__ == "__main__":
    ...
    for task in TEXT_TASKS["glue"]:
        ...
        if args.glue_lora:
            results_dir = "lora"
        preds_path = f"{args.model_name}/results/{results_dir}/{task}/predictions.txt"
        task_dicts["glue"][task] = make_task_dict("glue", task, preds_path)
    for task in TEXT_TASKS["blimp"]:
        preds_path = f"{args.model_name}/results/blimp/blimp_{task}_filtered_results.jsonl"
        task_dicts["blimp"][task] = make_task_dict("blimp", task, preds_path)
    for task in TEXT_TASKS["blimp_supplement"]:
        preds_path = f"{args.model_name}/results/blimp/blimp_supplement_{task}_results.jsonl"
        task_dicts["blimp_supplement"][task] = make_task_dict("blimp_supplement", task, preds_path)
    for task in TEXT_TASKS["ewok"]:
        preds_path = f"{args.model_name}/results/ewok/ewok_{task}_filtered_results.jsonl"
    ...
    if args.include_vision_tasks:
        ...
        task_dicts["vqa"]["vqa"] = make_task_dict("vqa", "vqa",
                                        f"{args.model_name}/results/vqa_filtered/vqa_val_filtered_results.jsonl")
        task_dicts["winoground"] = {}
        task_dicts["winoground"]["winoground"] = make_task_dict("winoground", "winoground",
                                                    f"{args.model_name}/results/winoground_filtered/winoground_results.jsonl")
        task_dicts["devbench"] = {}
    ...
    with gzip.open(f"{args.model_name}/results/{model_basename}_{preds_name}_predictions.json.gz", 'wt') as predictions_out:
